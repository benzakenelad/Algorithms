...
# extract the vector unique labels and a counter of the unique labels
...
# return the index of the max of values
def argmax(values):
    return max(enumerate(values), key=lambda x: x[1])[0]


# return the a list of indices that sort the values
def argsort(values):
    return sorted(range(len(values)), key=values.__getitem__)


# remove the col_ind colmun from data
